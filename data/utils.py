...
a3-dB bandwidth of 5Hz,also known asb and-rejection filter [paper]
'''
def notch_filter(ecg, freq=60.0, bw=5.0):
    samp_freq = freq / ( SAMPLING_RATE / 2)
    quality_factor = freq/bw
    b,a = iirnotch(samp_freq,quality_factor)
...
is then subtracted from the ECG signal to remove this artifact.
'''
...
def preprocess_ECG(ECG):
    ...
    for i in range(num_samples):
        ...
        preprocessed_leads.append(cleaned)

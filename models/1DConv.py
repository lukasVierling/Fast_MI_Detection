...
class LeadProcessingBlock1D(nn.Module):
    def __init__(self, num_leads, filters_per_lead, dilation1, dilation2):
        super().__init__()
        channels = num_leads * filters_per_lead
        self.first_layer = nn.Sequential(
            nn.Conv1d(channels, channels, 3, padding=dilation1, dilation=dilation1, groups=num_leads, bias=False),
            nn.BatchNorm1d(channels),
            nn.ReLU(inplace=True)
        )
        self.second_layer = nn.Sequential(
            nn.Conv1d(channels, channels, 3, padding=dilation2, dilation=dilation2, groups=num_leads, bias=False),
            nn.BatchNorm1d(channels),
            nn.ReLU(inplace=True)
        )
